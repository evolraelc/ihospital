...
angular.module('pagination', []).directive('tmPagination',[function(){
    return {
        ...
        template: '<div class="page-list">' +
            ...
            'Show <select ng-model="conf.itemsPerPage" ng-options="option for option in conf.perPageOptions "></select>entries' +
            '  Tatal  <strong>{{ conf.totalItems }}</strong> pcs' +
            '</div>' +
            '<div class="no-items" ng-show="conf.totalItems <= 0">No data at this time</div>' +
            '</div>',
        replace: true,
        scope: {
            conf: '='
