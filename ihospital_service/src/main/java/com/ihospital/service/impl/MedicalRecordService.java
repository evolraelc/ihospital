...
public class MedicalRecordService implements IMedicalRecordService {
    ...
    public List<AppointInfo> findAppointmentList(Long patientId) {
        ...
        for (Integer i = 0; i < departmentsOrderedByPatientId.size(); i++) {
            ...
            AppointmentExample.Criteria criteria2 = appointmentExample2.createCriteria();
            criteria2.andDeptIdEqualTo(departmentsOrderedByPatientId.get(i).getDepartId());
            criteria2.andIsValidEqualTo(true);
            appointmentExample2.setOrderByClause("`priority` ASC,`appoint_time` ASC");
            ...
            for (Integer j = 0; j < partAppointments.size(); j++) {
                if (partAppointments.get(i).getPatientId().equals(patientId)) {
                    num.add(i, j + 1);
                }
            }
        }
        ...
        for (Integer x = 0; x < appointList.size(); x++) {
            appointInfoList.add(x, new AppointInfo());

            //从departmentsOrderedByPatientId中获取的数据
            appointInfoList.get(x).setDepartmentName(departmentsOrderedByPatientId.get(x).getDepartName());
            appointInfoList.get(x).setAddress(departmentsOrderedByPatientId.get(x).getPosition());

            //从appointList中获取的数据
            appointInfoList.get(x).setTreatTime(appointList.get(x).getTreatTime());
            appointInfoList.get(x).setAppointTime(appointList.get(x).getAppointTime());
            appointInfoList.get(x).setType(appointList.get(x).getPriority());

            //从list num中获取数据
            appointInfoList.get(x).setNum(num.get(x));
        }
